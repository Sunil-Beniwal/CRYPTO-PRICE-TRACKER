...
import { formatMarketCap, formatPrice } from "../utility/formatter";
import {
  CartesianGrid,
  LineChart,
  ResponsiveContainer,
...
function CoinDetail(){
  ...
  useEffect(() => {
    setIsLoading(true); 
    // When the route parameter (id) changes, the component is not remounted, so useState is not reinitialized. Therefore, we manually reset isLoading to true inside useEffect to correctly represent a new data-fetch cycle. 
    loadCoinData();
    loadChartData();
  }, [id]);
...
// npm install recharts for price history chart